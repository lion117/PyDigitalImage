...
def ExtractSuperPixImage(tImage , tSegments = 100):
	...
	if os.path.exists(tImage) is False:
		print(u"%s file is not exist"%(tImage))
		return
	...
	orig = cv2.imread(tImage)
	vis = np.zeros(orig.shape[:2], dtype="float")
	# load the image and apply SLIC superpixel segmentation to it via
	# scikit-image
	image = io.imread(tImage)
	segments = slic(img_as_float(image), n_segments=tSegments,
					slic_zero=True)
...
class MainTest():
	@staticmethod
	def testExtractSuperPixImage():
		lImage = u"example1.png"
		lImage2 = u"example3.jpg"
		lSegement = 200
		ExtractSuperPixImage(lImage2,lSegement)



if __name__ == "__main__":
	print (os.getcwd())
	MainTest.testExtractSuperPixImage()